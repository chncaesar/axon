...
from axon.utilities import Printer
from axon.utilities.axon_json_encoder import AxonJSONEncoder
from axon.utilities.errors import DatabaseError, DatabaseOperationError
from axon.utilities.paths import db_storage_path
...
class KickoffTaskOutputsSQLiteStorage:
    """
    The SQLite storage class for kickoff task outputs storage.
    """
        ...
        try:
            ...
            """,
                    (
                        ...
                        json.dumps(output, cls=AxonJSONEncoder),
                        task_index,
                        json.dumps(inputs, cls=AxonJSONEncoder),
                        was_replayed,
                    ),
                )
                conn.commit()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for key, value in kwargs.items():
                    fields.append(f"{key} = ?")
                    values.append(
                        json.dumps(value, cls=AxonJSONEncoder)
